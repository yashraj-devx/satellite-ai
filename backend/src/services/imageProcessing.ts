...
import { AnalysisLayer, LandCoverMetric, RemoteSensingIntent } from '../types';

export interface ProcessedAlgorithmResult {
 ...
 */
function applyColormap(value: number, map: 'ndvi' | 'jet' | 'turbo' | 'viridis' | 'inundation' | 'urban' | 'radar'): [number, number, number, number] {
...
export class ImageProcessingService {
   ...
   */
  private static async getRawPixels(input: Buffer | string): Promise<{
    data: Buffer;
    width: number;
    height: number;
    channels: number;
   ...
   */
  private static async createDataUrl(buffer: Buffer, width: number, height: number): Promise<string> {
   ...
   */
  public static async analyzeWater(imagePath: string): Promise<ProcessedAlgorithmResult> {
   ...
   */
  public static async analyzeVegetation(imagePath: string): Promise<ProcessedAlgorithmResult> {
    const { data, width, height } = await this.getRawPixels(imagePath);
    const totalPixels = width * height;
   ...
   */
  public static async analyzeChangeDetection(beforePath: string, afterPath: string): Promise<ProcessedAlgorithmResult> {
   ...
   */
  public static async analyzeUrban(imagePath: string): Promise<ProcessedAlgorithmResult> {
    const { data, width, height } = await this.getRawPixels(imagePath);
    const totalPixels = width * height;
   ...
   */
  public static async analyzeSAR(imagePath: string): Promise<ProcessedAlgorithmResult> {
    const { data, width, height } = await this.getRawPixels(imagePath);
    const totalPixels = width * height;
   ...
   */
  public static async analyzeLandCover(imagePath: string): Promise<ProcessedAlgorithmResult> {
    const { data, width, height } = await this.getRawPixels(imagePath);
    const totalPixels = width * height;
    ...
    for (let i = 0; i < totalPixels; i++) {
      ...
      if ((bNorm > rNorm * 1.2 && gNorm > rNorm * 0.95 && bNorm > 0.25) || (bNorm > 0.4 && rNorm < 0.2)) {
        ...
        counts.water++;
        classMap[offset] = 0; classMap[offset + 1] = 162; classMap[offset + 2] = 255; classMap[offset + 3] = 200;
      } else if (gNorm > rNorm * 1.15 && gNorm > bNorm * 1.15 && gNorm < 0.55) {
        // Dense Forest (Deep Green)
        counts.forest++;
        classMap[offset] = 5; classMap[offset + 1] = 122; classMap[offset + 2] = 85; classMap[offset + 3] = 200;
      } else if (gNorm > rNorm && gNorm > bNorm) {
        // Agriculture / Grassland (Lime Green)
        counts.agriculture++;
        classMap[offset] = 49; classMap[offset + 1] = 196; classMap[offset + 2] = 141; classMap[offset + 3] = 190;
      } else if (Math.abs(r - g) < 25 && Math.abs(g - b) < 25 && r > 110) {
        // Urban / Built-up (Red/Terracotta)
        counts.urban++;
        classMap[offset] = 224; classMap[offset + 1] = 36; classMap[offset + 2] = 36; classMap[offset + 3] = 200;
      } else if (rNorm > 0.45 && gNorm > 0.35 && bNorm < 0.35) {
        // Bare Soil / Sand (Amber/Gold)
        counts.soil++;
        classMap[offset] = 227; classMap[offset + 1] = 160; classMap[offset + 2] = 8; classMap[offset + 3] = 190;
      } else {
        // Infrastructure / Roads (Slate Gray)
        counts.roads++;
        classMap[offset] = 107; classMap[offset + 1] = 114; classMap[offset + 2] = 128; classMap[offset + 3] = 180;
      }
    }
    ...
    const breakdown: LandCoverMetric[] = [
  ...
  ): Promise<ProcessedAlgorithmResult> {
