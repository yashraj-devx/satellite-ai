...
exports.SampleDataService = exports.SampleDataGenerator = void 0;
...
class SampleDataGenerator {
     ...
     */
    static async generateSampleImages(outputDir) {
        if (!fs_1.default.existsSync(outputDir)) {
            fs_1.default.mkdirSync(outputDir, { recursive: true });
        }
        const size = 512;
        // 1. Sundarbans Optical (Estuary & Mangroves)
        const sundarbansPath = path_1.default.join(outputDir, 'sundarbans_sentinel2.jpg');
        if (!fs_1.default.existsSync(sundarbansPath)) {
            ...
            await (0, sharp_1.default)(buf, { raw: { width: size, height: size, channels: 3 } }).jpeg({ quality: 90 }).toFile(sundarbansPath);
        }
        // 2. Sentinel-1 SAR Radar (Brahmaputra Flood Basin)
        const sarPath = path_1.default.join(outputDir, 'brahmaputra_sar_s1.jpg');
        if (!fs_1.default.existsSync(sarPath)) {
            const buf = Buffer.alloc(size * size * 3);
            for (let y = 0; y < size; y++) {
                for (let x = 0; x < size; x++) {
                    const idx = (y * size + x) * 3;
                    ...
                    if (isFloodWater) {
                        ...
                        if (isDoubleBounce) {
                            buf[idx] = 250;
                            buf[idx + 1] = 240;
                            buf[idx + 2] = 220; // High radar reflection
                        }
                        else {
            ...
            await (0, sharp_1.default)(buf, { raw: { width: size, height: size, channels: 3 } }).jpeg({ quality: 90 }).toFile(sarPath);
        }
        // 3. Bengaluru 2018 (Before: More Greenery)
        const bglr2018Path = path_1.default.join(outputDir, 'bengaluru_2018_before.jpg');
        if (!fs_1.default.existsSync(bglr2018Path)) {
            const buf = Buffer.alloc(size * size * 3);
            for (let y = 0; y < size; y++) {
                for (let x = 0; x < size; x++) {
                    const idx = (y * size + x) * 3;
            ...
            await (0, sharp_1.default)(buf, { raw: { width: size, height: size, channels: 3 } }).jpeg({ quality: 90 }).toFile(bglr2018Path);
        }
        // 4. Bengaluru 2024 (After: Urban Expansion & Green Loss)
        const bglr2024Path = path_1.default.join(outputDir, 'bengaluru_2024_after.jpg');
        if (!fs_1.default.existsSync(bglr2024Path)) {
            const buf = Buffer.alloc(size * size * 3);
            for (let y = 0; y < size; y++) {
                for (let x = 0; x < size; x++) {
                    const idx = (y * size + x) * 3;
            ...
            await (0, sharp_1.default)(buf, { raw: { width: size, height: size, channels: 3 } }).jpeg({ quality: 90 }).toFile(bglr2024Path);
        }
        // 5. Punjab Agriculture (Crop NDVI patterns)
        const punjabPath = path_1.default.join(outputDir, 'punjab_agriculture_s2.jpg');
        if (!fs_1.default.existsSync(punjabPath)) {
            const buf = Buffer.alloc(size * size * 3);
            const grid = 64;
            for (let y = 0; y < size; y++) {
                for (let x = 0; x < size; x++) {
                    ...
                    if (isCanal) {
                        ...
                        buf[idx] = 15;
                        buf[idx + 1] = 160 + Math.floor(Math.random() * 20);
                        buf[idx + 2] = 40;
                    }
                    else if (fieldType === 1) {
                        // Moderate crop (Lime)
                        buf[idx] = 60;
                        buf[idx + 1] = 180 + Math.floor(Math.random() * 20);
                        buf[idx + 2] = 50;
                    }
                    else if (fieldType === 2) {
            ...
            await (0, sharp_1.default)(buf, { raw: { width: size, height: size, channels: 3 } }).jpeg({ quality: 90 }).toFile(punjabPath);
        }
    }
}
exports.SampleDataGenerator = SampleDataGenerator;
