...
class AIService {
     ...
     */
    static classifyIntent(query, hasDualImages = false, hasSar = false) {
     ...
     */
    static fileToGenerativePart(filePath, mimeType = 'image/jpeg') {
        return {
            inlineData: {
     ...
     */
    static async analyzeWithAI(query, intent, algoResult, images, metadata) {
        ...
        if (apiKey && apiKey !== 'your_api_key_here' && apiKey.trim().length > 10) {
            try {
                ...
                const parts = [];
                if (images.primaryPath && fs_1.default.existsSync(images.primaryPath)) {
                    parts.push(this.fileToGenerativePart(images.primaryPath));
                }
                if (images.beforePath && fs_1.default.existsSync(images.beforePath)) {
                    parts.push(this.fileToGenerativePart(images.beforePath));
                }
                if (images.afterPath && fs_1.default.existsSync(images.afterPath)) {
                    parts.push(this.fileToGenerativePart(images.afterPath));
                }
                if (images.sarPath && fs_1.default.existsSync(images.sarPath)) {
...
Return a valid, raw JSON object (NO markdown backticks, NO extra text) with the following structure:
{
  ...
  "findings": [
     ...
     */
    static generateFallbackResponse(query, intent, algoResult, metadata) {
